...
function preload(){
trexAnimation.loadImage("trex.jpg");

}
function setup(){

      var trex = createSprite(200,380,20,50);
      trex.addAnimation(trexAnimation);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    //scoring working with the frame rate then the frame count. frame rate is the speed of the frame count.
    ...
    // infinte game world to reset the ground
    if (ground.x < 0){
      ground.x = ground.width/2;
    }
...
// the reset icon properties and function
function reset(){
 // changes into play state
  gameState=PLAY;
  // destroys obstacles and clouds to restart the game.
  ObstaclesGroup.destroyEach();
  CloudsGroup.destroyEach()
  ;
  trex.setAnimation("trex");
  
  // makes the game over invisible during the play state
  restart.visible=false;
  gameOver.visible=false;
  // changes count to 0 rather then the continual of the previous score.
  count=0;
}
...
function spawnClouds() {
  ...
  if (World.frameCount % 60 === 0) {
    ...
    cloud.velocityX = -3;
     ...
     //assign lifetime to the variable.(distance/velocity)
    cloud.lifetime = 134;
